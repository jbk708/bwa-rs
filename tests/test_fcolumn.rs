...
fn test_simple_pattern_search() {
    ...
    for pattern in patterns {
        ...
        println!("Pattern '{}': range=[{}, {}), positions={:?}", pattern, left, right, positions);
        ...
        assert_eq!(sorted_positions, expected, "Pattern '{}' positions mismatch", pattern);
    }
}
...
fn test_large_pattern_search() {
    ...
    ref_bytes.splice(pattern_pos as usize..pattern_pos as usize + pattern.len(), pattern.bytes());
    ...
    eprintln!("Pattern '{}' at pos {}: range=[{}, {}), positions={:?}", 
        pattern, pattern_pos, left, right, positions);
    
    assert!(!positions.is_empty(), "Should find pattern");
    assert!(positions.contains(&pattern_pos), "Should find at position {}", pattern_pos);
}
    ...
    for (i, &pos) in index.sa.as_slice().iter().enumerate() {
        if pos < ref_seq.len() as u32 {
            ...
            if suffix.starts_with("G") {
                eprintln!("  SA[{}] = {} -> '{}'", i, pos, &suffix[..10.min(suffix.len())]);
            }
        } else {
...
fn test_debug_wavelet_rank() {
    ...
    for i in 0..=bwt.len() {
        ...
        if wt_rank != expected {
            eprintln!("MISMATCH at i={}: wt_rank={}, expected={}", i, wt_rank, expected);
        }
    }
...
fn test_debug_search_trace() {
    ...
    for (step, &c) in pattern.iter().rev().enumerate() {
        let char_name = match c { 0 => "A", 1 => "C", 2 => "G", 3 => "T", 4 => "$", _ => "?" };
        ...
        eprintln!("  F[{}]={}, F[{}]-1={}", c, index.f_column[c as usize], c, f_c);
...
fn test_debug_sa_ordering() {
    ...
    // All suffixes
    let suffixes: Vec<(&[u8], usize)> = (0..seq.len())
        .map(|i| (&seq[i..], i))
        .collect();
...
fn test_debug_f_column_with_4() {
    ...
    let ref_seq = "ACAC";
    let sequence: Vec<u8> = ref_seq.bytes()
        .map(|b| match b {
            b'A' => 0,
            b'C' => 1,
            b'G' => 2,
...
fn test_debug_bwt_acac() {
    ...
    for (i, &c) in bwt.iter().enumerate() {
        ...
        println!("BWT[{}] = {} ({}) for SA[{}] = {}", i, c, char_name, i, suffix_pos);
    }
...
fn test_debug_fm_search_acgt() {
    ...
    for (step, &c) in pattern.iter().rev().enumerate() {
        ...
        eprintln!("\nStep {}: c={}", step, c);
        eprintln!("  F[{}] = {}, F[{}]-1 = {}", c, fm_index.f_column[c as usize], c, f_c);
        eprintln!("  occ({}, {}) = {}", c, left, occ_left);
        eprintln!("  occ({}, {}) = {}", c, right, occ_right);
