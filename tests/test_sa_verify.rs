...
fn test_verify_sa_and_fcolumn() {
    ...
    for i in 0..index.sa.len() {
        if let Some(pos) = index.sa.get(i) {
            ...
            println!(
                ...
                i,
                pos,
                suffix.chars().take(10).collect::<String>(),
                first_char
            );
    ...
    if !a_positions.is_empty() {
        println!(
            "  First A at SA index {}, so F[0] should be {} (1-indexed)",
            a_positions[0].0,
            a_positions[0].0 + 1
        );
    }
    if !c_positions.is_empty() {
        println!(
            "  First C at SA index {}, so F[1] should be {} (1-indexed)",
            c_positions[0].0,
            c_positions[0].0 + 1
        );
    }
    if !g_positions.is_empty() {
        println!(
            "  First G at SA index {}, so F[2] should be {} (1-indexed)",
            g_positions[0].0,
            g_positions[0].0 + 1
        );
    }
