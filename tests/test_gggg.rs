...
fn test_gggg_search() {
    ...
    println!(
        "Search for GGGG: range=[{}, {}), positions={:?}",
    ...
    for pos in &positions {
        ...
        println!(
            "  Position {}: suffix starts with '{}'",
            pos,
            &suffix[..10.min(suffix.len())]
        );
        assert!(
            suffix.starts_with("GGGG"),
            "Position {} should start with GGGG",
            pos
        );
    }
...
fn test_ggggaaaaacccc_search() {
    ...
    println!(
        "Search for GGGGAAAAACCCC: range=[{}, {}), positions={:?}",
        left, right, positions
    );
    ...
    for pos in &positions {
        let suffix = &ref_seq[*pos as usize..];
        assert!(
            suffix.starts_with("GGGGAAAAACCCC"),
            "Position {} should start with GGGGAAAAACCCC",
            pos
        );
    }
