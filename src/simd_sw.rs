    ...
    pub fn detect() -> Self {
        #[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
        {
            if is_x86_feature_detected!("avx512f") && is_x86_feature_detected!("avx512bw") {
                return Self { enabled: true, use_avx512: true, use_avx2: false, lanes: 16 };
            }
            if is_x86_feature_detected!("avx2") {
                return Self { enabled: true, use_avx512: false, use_avx2: true, lanes: 8 };
            }
        }
        Self { enabled: false, use_avx512: false, use_avx2: false, lanes: 1 }
    }
}
...
pub fn nw_score(query: &[u8], reference: &[u8], scoring: &Scoring) -> i32 {
    ...
    if config.enabled && config.use_avx2 {
        ...
        unsafe { return avx2_nw_score(query, reference, scoring); }
    }
...
fn scalar_nw_score(query: &[u8], reference: &[u8], scoring: &Scoring) -> i32 {
    ...
    for i in 1..=q_len {
        ...
        for j in 1..=r_len {
            ...
            let match_val = if is_match { scoring.match_score } else { -scoring.mismatch_penalty };
...
pub fn extend_forward_simd(
    query: &[u8], reference: &[u8], start_pos: usize, scoring: &Scoring, bandwidth: usize,
) -> SeedExtension {
    let config = SimdConfig::detect();
    if config.enabled && config.use_avx2 {
        #[cfg(target_arch = "x86_64")]
        unsafe { return avx2_extend_forward(query, reference, start_pos, scoring, bandwidth); }
    }
...
fn scalar_extend_forward(
    ...
    if q_len == 0 || ref_len == 0 {
        return SeedExtension { score: 0, query_end: 0, ref_end: start_pos, cigar: Cigar::new() };
    }
    ...
    dp_curr[0] = 0;
    for j in 1..=max_j { dp_curr[j] = dp_curr[j - 1].saturating_sub(scoring.gap_extend); }
    std::mem::swap(&mut dp_prev, &mut dp_curr);
    ...
    for i in 1..=q_len {
        ...
        for j in j_start..=j_end {
            ...
            if ref_idx >= reference.len() { break; }
            let is_match = query[i - 1] == reference[ref_idx];
            let match_val = if is_match { scoring.match_score } else { -scoring.mismatch_penalty };
            let diag = if j > j_start { dp_prev[j - 1].saturating_add(match_val) } else { match_val };
            ...
            dp_curr[j] = val;
            if val > best_score { best_score = val; best_i = i; best_j = j; }
        }
        std::mem::swap(&mut dp_prev, &mut dp_curr);
    }

    let cigar = traceback(&dp_prev, query, reference, start_pos, best_i, best_j, scoring);
    SeedExtension { score: best_score, query_end: best_i, ref_end: start_pos + best_j, cigar }
}

fn traceback(
    dp_prev: &[i32], query: &[u8], reference: &[u8], ref_start: usize,
    mut i: usize, mut j: usize, scoring: &Scoring,
) -> Cigar {
    let mut ops = Vec::new();
    ...
    while i > 0 && j > 0 {
        ...
        let diag = if rel_j > 0 { dp_prev[rel_j - 1] } else { i32::MIN / 4 };
        if curr == diag + delta {
            ops.push(if is_match { CigarOp::Eq } else { CigarOp::X });
            i -= 1; j -= 1;
        } else if curr == dp_prev[rel_j] - gap_open {
            ops.push(CigarOp::D); i -= 1;
        } else {
            ops.push(CigarOp::I); j -= 1;
        }
    }
    while i > 0 { ops.push(CigarOp::D); i -= 1; }
    while j > 0 { ops.push(CigarOp::I); j -= 1; }
    ops.reverse();
    compress_cigar(ops)
}
...
fn compress_cigar(ops: Vec<CigarOp>) -> Cigar {
    ...
    if ops.is_empty() { return cigar; }
    let mut current_op = ops[0];
    let mut current_len = 1u32;
    for op in ops.iter().skip(1) {
        if *op == current_op { current_len += 1; }
        else { cigar.push(current_op, current_len); current_op = *op; current_len = 1; }
    }
...
mod avx2_impl {
    ...
    pub unsafe fn avx2_nw_score(query: &[u8], reference: &[u8], scoring: &Scoring) -> i32 {
        let (q_len, r_len) = (query.len(), reference.len());
        if q_len == 0 || r_len < LANES { return scalar_nw_score(query, reference, scoring); }
        ...
        for j in 1..=r_len { dp_curr[j] = dp_curr[j - 1].saturating_add(gap_extend); }
        std::mem::swap(&mut dp_prev, &mut dp_curr);
        ...
        for i in 1..=q_len {
            ...
            for b in 0..num_blocks {
                ...
                for k in 0..LANES { rv[k] = reference.get(base + k).copied().unwrap_or(0) as i32; }
        ...
        dp_prev[..r_len + 1].iter().copied().max().unwrap_or(0).max(0)
    }
    ...
    pub unsafe fn avx2_extend_forward(
        ...
        dp_curr[0] = 0;
        for j in 1..=max_j { dp_curr[j] = dp_curr[j - 1].saturating_sub(gap_extend); }
        std::mem::swap(&mut dp_prev, &mut dp_curr);
        ...
        for i in 1..=q_len {
            ...
            while j + LANES <= i_max_j {
                ...
                let result = (diag_v + scores).max(left_v + gap_extend_v).max(prev_v + gap_open_v);
                result.store(&mut dp_curr[base..base + LANES]);
            ...
            for jj in j..=i_max_j {
                ...
                let s = if is_match { match_s } else { mismatch_s };
                let diag = if jj > i_min_j { dp_prev[jj - 1].saturating_add(s) } else { s };
                let up = dp_prev[jj].saturating_sub(gap_open);
                let left = dp_curr[jj - 1].saturating_sub(gap_extend);
                let val = diag.max(up).max(left);
                dp_curr[jj] = val;
                if val > best_score { best_score = val; best_i = i; best_j = jj; }
            }
            std::mem::swap(&mut dp_prev, &mut dp_curr);
        }
...
mod tests {
    ...
    fn test_compress_cigar() {
        let ops = vec![CigarOp::Eq, CigarOp::Eq, CigarOp::Eq, CigarOp::X, CigarOp::X];
